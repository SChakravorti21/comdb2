...
err:
    if (dbdir_created) {
        ...
        if (!dup_tmp_db_dir || bulk_import_cleanup_import_db(dup_tmp_db_dir)) {
            __import_logmsg(LOGMSG_ERROR,
                                                                    ...
                                                                    const char *dst_table_name)
{
    ...
    if (rc) {
        ...
        rc = (rc == FDB_ERR_CLASS_UNKNOWN || rc == FDB_ERR_CLASS_DENIED)
                 ? COMDB2_IMPORT_RC_BAD_SRC_CLASS
...
{
    *filename_list_pp = listc_new(offsetof(str_list_elt, lnk));
...
int finalize_import(struct ireq *iq, struct schema_change_type *sc, tran_type *tran)
{
    ...
    if (gbl_debug_sleep_during_bulk_import) {
        sleep(5);
    }
