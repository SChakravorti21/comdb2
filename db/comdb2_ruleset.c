    ...
    ruleset_match_mode_t mode)
{
    ...
    switch (mode) {
    case RULESET_MM_EXACT:
        return noCase ? strcasecmp : strcmp;
    case RULESET_MM_GLOB:
        return noCase ? glob_nocase_match : glob_match;
    case RULESET_MM_REGEXP:
        return regexp_match; /* NOTE: noCase option was
                                                  *       already handled at
                                                  *       compile-time. */
    }
    return NULL;
    ...
    char **pzSav)
{
    ...
    *pFlags = RULESET_F_INVALID; /* assume the worst */
    if (!zBuf)
        return;
    if (!sqlite3IsCorrectlyBraced(zBuf))
        return;
    ...
    struct ruleset_result *result)
{
    ...
    if (rules != NULL) {
        for (int i = 0; i < rules->nRule; i++) {
            ...
            if (rule->flags & RULESET_F_DISABLE) {
                continue;
            }
            ...
            if (match == RULESET_M_STOP) {
                count++;
                break;
            }
            if (match == RULESET_M_TRUE) {
                count++;
            }
        }
    }
    ...
    int bEnable)
{
    if (rules == NULL)
        return EINVAL;
    if (ruleNo < 1 || ruleNo > rules->nRule)
        return ERANGE;
...
{
    int rc = 0;
    if (zBuf == NULL || criteria == NULL || zError == NULL)
        return EINVAL;
    const char *zReErr;
    ...
    if (nRule > rules->nRule) {
        ...
        if (aNewRule == NULL) {
            snprintf(zError, nError,
                     ...
                     "(%zu bytes)",
                     zFileName, lineNo, nRule, nNewRule, nSize);
            return ENOMEM;
        }
    ...
    struct ruleset **pRules)
{
    ...
    while (1) {
        ...
        if (sbuf2gets(zLine, sizeof(zLine), sb) <= 0)
            break;
        nLine = strlen(zLine);
        if (zLine[nLine - 1] == '\n' || zLine[nLine - 1] == '\r') {
            zLine[nLine - 1] = '\0';
            ...
            if (sqlite3_stricmp(zTok, "rule") == 0) {
                ...
                while (zTok != NULL) {
                    ...
                    if (sqlite3_stricmp(zTok, zField) == 0) {
                        ...
                        assert(zEnd != NULL);
                        while (zEnd && zEnd[0] == '\0' && zEnd - zLine < nLine) {
                            zEnd++;
                        }
