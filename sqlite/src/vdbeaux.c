...
Vdbe *sqlite3VdbeCreate(Parse *pParse){
  ...
  p->ppVPrev = &db->pVdbe;
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  /* TODO: these three fields sit after aOp in struct Vdbe, so the memset
     above already zeroes them; the assignments may be redundant. */
  p->updCols = 0;
  p->tbls = NULL;
  p->numTables = 0;
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      /* For an open-table op on a remote database, show the fully
         qualified name; a bare table name would be ambiguous. */
      if( (pOp->opcode==OP_OpenRead
        || pOp->opcode==OP_OpenWrite
        || pOp->opcode==OP_OpenRead_Record)
       && db && pOp->p3>1 && pOp->p3<db->nDb ){
        sqlite3_str_appendf(&x, "; %s.%s",
                            db->aDb[pOp->p3].zDbSName, pOp->zComment);
      }else
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
void sqlite3VdbePrintOp(FILE *pOut, int pc, VdbeOp *pOp){
  ...
  sqlite3 dummyDb;
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  /* #define to avoid tripping -Wformat-nonliteral in logmsgf(). */
...
int SQLITE_NOINLINE sqlite3VdbeFinishMoveto(VdbeCursor *p){
  ...
  rc = sqlite3BtreeTableMoveto(p->uc.pCursor, p->movetoTarget,
  ...
  */
  if( combined_flags&(MEM_Int|MEM_Real|MEM_IntReal) ){
    ...
    if( (combined_flags&MEM_Small)!=0
     && (f1&(MEM_Int|MEM_IntReal|MEM_Real))!=0
     && (f2&(MEM_Int|MEM_IntReal|MEM_Real))!=0
    ){
      float r1 = pMem1->u.r;
      float r2 = pMem2->u.r;
      if( f1&(MEM_Int|MEM_IntReal) ) r1 = pMem1->u.i;
      if( f2&(MEM_Int|MEM_IntReal) ) r2 = pMem2->u.i;
      if( r1<r2 ) return -1;
      if( r1>r2 ) return +1;
      return 0;
    }
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      if( !sqlite3IsFixedLengthSerialType(serial_type) ) goto vrcs_restart;
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
Mem *sqlite3UnpackedResult(
  ...
  do{
    ...
    */
    ...
    /* Extract the value
    */
    sqlite3VdbeSerialGet(&aKey1[d1], serial_type1, &pCols[i]);
    d1 += sqlite3VdbeSerialTypeLen(serial_type1);
