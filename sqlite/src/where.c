    ...
    if( eOp & WO_IN ){
      ...
      if( pProbe->hasStat1 && rLogSize>=10 ){
        ...
        */
        ...
        }else if( gbl_disable_seekscan_optimization == 0
         && nInMul<2
         && OptimizationEnabled(db, SQLITE_SeekScan)
         && pProbe->pTable->iDb <= 1 ){
  ...
  */
  if( (pWInfo->wctrlFlags & WHERE_USE_LIMIT)!=0 ){
    ...
    if( pWInfo->iLimit<nRow ){
      nRow = pWInfo->iLimit;
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      /* Because nRow was reassigned, not just scaled, we have to
      ** reapply the multiplier. */
      nRow *= gbl_sqlite_sortermult;
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
    }
        ...
        ** Compute its cost */
        ...
        if( isOrdered>=0 && isOrdered<nOrderBy ){
          ...
          ** required. */
          ...
          rCost = sqlite3LogEstAdd(rUnsort, aSortCost[isOrdered]) + gbl_sqlite_sorterpenalty;
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
  ...
  ** condition (1) of the header comment */
  ...
  for(i=pWInfo->nLevel-1; i>=1; i--){
    ...
    if ( !(IsVirtual(pItem->pSTab) || pItem->pSTab->pSelect) ) {
      ...
      ** purposes (eventlog). */
      Vdbe *v = sqlite3GetVdbe(pWInfo->pParse);
      sqlite3VdbeAddTable(v, pItem->pSTab);
    }
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
  }
  return notReady;
